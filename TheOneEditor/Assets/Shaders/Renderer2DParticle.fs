...
{
    return light.Color * light.Intensity * 0.1;
}

vec3 CalcPointLight(PointLight light, vec3 texColor)
{
    float distance = length(light.Position - particlePos);
    if(distance > light.Radius) return vec3(0.0);

    float attenuation = 1.0 / (1.0 + light.Linear * distance + light.Quadratic * distance * distance);
    return texColor * light.Color;// * light.Intensity * 500.0 * attenuation;
}

vec3 CalcSpotLight(SpotLight light, vec3 texColor, vec3 particleNormal)
{
    float distance = length(light.Position - particlePos);
    if(distance > light.Radius) return vec3(0.0);

    // diffuse
    vec3 lightDir = normalize(light.Position - particlePos);
    float diffuse = abs(dot(particleNormal, lightDir));

    float attenuation = 1.0 / (1.0 + light.Linear * distance + light.Quadratic * distance * distance);

    return texColor * diffuse * light.Color * light.Intensity * 500.0 * attenuation;
}
...
void main()
{
    ...
    vec3 particleNormal = normalize(camPos - particlePos);

    //for(int i = 0; i < MAX_DIRECTIONAL_LIGHTS; i++)
    //texColor.xyz *= CalcDirLight(u_DirLight[i]);

    for(int i = 0; i < MAX_POINT_LIGHTS; i++)
    texColor.xyz += CalcPointLight(u_PointLights[i], texColor.xyz);

    // for(int i = 0; i < MAX_SPOT_LIGHTS; i++)
    // texColor.xyz += CalcSpotLight(u_SpotLights[i], texColor.xyz, particleNormal);
