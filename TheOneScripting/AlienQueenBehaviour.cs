...
public class AlienQueenBehaviour : MonoBehaviour
{
    ...
    public override void Update()
    {
        if (currentState == States.Dead)
        {
            ...
            playerGO.source.StopAudio(IAudioSource.Events.UI_A_MENU);
            playerGO.source.PlayAudio(IAudioSource.Events.UI_CLICK);
        }
    ...
    private void AcidSpit()
    {
        ...
        if (delayTimer >= delay)
        {
            //InternalCalls.InstantiateBullet(attachedGameObject.transform.position + attachedGameObject.transform.forward * (attachedGameObject.GetComponent<ICollider2D>().radius + 12.5f) + height,
            //                                attachedGameObject.transform.rotation);
            attachedGameObject.source.PlayAudio(IAudioSource.Events.E_QUEEN_SPIT);
            delayTimer = 0.0f;
            shotsCounter++;
    ...
    private void XenoSpawn()
    {
        ...
        attachedGameObject.source.PlayAudio(IAudioSource.Events.E_X_ADULT_SPAWN);
        ResetState();
    }
    ...
    private void AcidBomb()
    {
        //InternalCalls.InstantiateBullet(attachedGameObject.transform.position + attachedGameObject.transform.forward * (attachedGameObject.GetComponent<ICollider2D>().radius + 12.5f) + height,
        //                                    attachedGameObject.transform.rotation);
        attachedGameObject.source.PlayAudio(IAudioSource.Events.E_QUEEN_BOMB);
        ResetState();
    }
    ...
    private void GiantJump(bool isJumpingForward)
    {
        ...
        if (up)
        {
            if (maxHeight >= currentHeight)
            {
                ...
                attachedGameObject.source.PlayAudio(IAudioSource.Events.E_QUEEN_JUMP);
                currentHeight += 10.0f;
            }
            else
            {
